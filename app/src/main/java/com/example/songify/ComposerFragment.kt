...
class ComposerFragment : Fragment() {
    ...
    private lateinit var rvComposers: RecyclerView
    private lateinit var swipeRefreshLayout: SwipeRefreshLayout
    private lateinit var adapter: ComposerAdapter
    private var allComposers: Map<String, List<Song>> = emptyMap()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_composer, container, false)
        rvComposers = view.findViewById(R.id.rvComposers)
        swipeRefreshLayout = view.findViewById(R.id.swipeRefreshLayoutComposers)
        rvComposers.layoutManager = GridLayoutManager(requireContext(), 3)

        fetchAndGroupComposers()
    ...
    private fun fetchAndGroupComposers() {
        ...
        service.getSongs(BuildConfig.SONGIFY_API_KEY, token).enqueue(object : Callback<SongResponse> {
            override fun onResponse(call: Call<SongResponse>, response: Response<SongResponse>) {
                ...
                if (response.isSuccessful) {
                    ...
                    allComposers = allSongs.flatMap { song ->
                        (song.composers ?: listOf("Unknown")).map { composer -> composer to song }
                    }.groupBy({ it.first }, { it.second }).toSortedMap()

                    adapter = ComposerAdapter(allComposers) { composerName, songs ->
                        showComposerSongsDetail(composerName, songs)
                    }
                    rvComposers.adapter = adapter
                }
            }
    ...
    fun filterComposers(query: String) {
        val filteredComposers = if (query.isEmpty()) {
            allComposers
        } else {
            allComposers.filter { it.key.contains(query, ignoreCase = true) }
        }
        adapter.updateComposers(filteredComposers)
    }

    private fun showComposerSongsDetail(composerName: String, songs: List<Song>) {
        ...
        rv.adapter = SongAdapter(sortedSongs, { selectedSong ->
            ...
            PlaylistRepository.addToPlaylist(song)
            Toast.makeText(requireContext(), "Song added to playlist", Toast.LENGTH_SHORT).show()
        }, { song ->
            PlaylistRepository.removeFromPlaylist(song)
